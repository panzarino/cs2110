...
void drawSquareImage(const u16 *image, int img_dimension)
{
    for (int i = 0; i < img_dimension; i++) {
        ...
        DMA[3].dst = videoBuffer + (i * img_dimension) + (i * (240 - img_dimension));
...
{
    for (int i = 0; i < img_dimension; i++) {
        DMA[3].src = image + (i * img_dimension) + img_dimension - 1;
        DMA[3].dst = videoBuffer + (i * img_dimension) + (i * (240 - img_dimension));
...
{
    for (int i = 0; i < img_dimension; i++) {
        DMA[3].src = image + ((img_dimension - i - 1) * img_dimension);
        DMA[3].dst = videoBuffer + (i * img_dimension) + (i * (240 - img_dimension));
        DMA[3].cnt = (img_dimension) | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT | DMA_ON;
    }
}
...
{
    for (int i = 0; i < img_dimension; i++) {
        DMA[3].src = image + ((img_dimension - i - 1) * img_dimension) + img_dimension - 1;
        DMA[3].dst = videoBuffer + (i * img_dimension) + (i * (240 - img_dimension));
        DMA[3].cnt = (img_dimension) | DMA_SOURCE_DECREMENT | DMA_DESTINATION_INCREMENT | DMA_ON;
    }
}
